...
type Proxy struct {
	...
	// ResultV fork: registry of live sockets (client conns + tunnel upstream
	// conns) that Close() force-closes. Upstream's Close() only waits on
	// conns.Wait(), but passthrough CONNECT tunnels sit in io.Copy pairs that
	// never observe `closing` — their only exit is the absolute read deadline
	// (up to `timeout`, 5 minutes), which stalled the whole shutdown.
...
func NewProxy(config Config) *Proxy {
	...
	// ResultV fork: Config.Dial overrides BOTH upstream dial paths — the
	// CONNECT/passthrough dialer above AND the http.Transport used for
	// MITM'd / plain-HTTP requests (upstream v0.2.1 exposes no hook for the
	// latter). See README-FORK.md.
	if config.Dial != nil {
		proxy.dial = config.Dial
		if t, ok := proxy.transport.(*http.Transport); ok {
			t.Proxy = nil
			t.DialContext = func(_ context.Context, network, addr string) (net.Conn, error) {
				return config.Dial(network, addr)
			}
		}
	}
...
func (p *Proxy) Close() {
	...
	// ResultV fork: force-close every live socket so tunnel io.Copy pairs
	// unblock immediately instead of running out their read deadline.
...
// trackConn registers a live socket for force-close on Close(). The
// post-registration Closing() check covers the race where Close() snapshots
// the registry while this conn is still being dialed/accepted.
...
// untrackConn removes a socket registered with trackConn.
...
func (p *Proxy) handleConnection(ctx *Context) {
	...
	// ResultV fork: register the client socket so Close() can sever it.
	p.trackConn(ctx.conn)
	defer p.untrackConn(ctx.conn)
	if p.Closing() {
		return
	}
...
func (p *Proxy) handleTunnel(session *Session) error {
	...
	// ResultV fork: register the upstream socket so Close() unblocks the
	// io.Copy pair below even when both ends are idle.
	p.trackConn(conn)
...
func (p *Proxy) handleConnect(session *Session) error {
	...
	if remoteConn != nil {
		defer remoteConn.Close()
		// ResultV fork: register the upstream socket so Close() unblocks the
		// passthrough io.Copy pair below even when both ends are idle.
		p.trackConn(remoteConn)
		defer p.untrackConn(remoteConn)
	}
	if err != nil {
		log.Error("id=%s: failed to connect to %s: %v", session.ID(), session.req.URL.Host, err)
		p.raiseOnError(session, err)
...
func (p *Proxy) canMITM(hostname string) bool {
	...
	return !p.isTLSException(hostname)
}

// isTLSException reports whether hostname is covered by the MITM exception list,
// matching both the exact host and any subdomain of a listed domain. Suffix
// matching is required: exceptions are registered as bare domains
// (googleapis.com, youtube.com) but real traffic hits subdomains
// (youtubei.googleapis.com, www.youtube.com). Without it those subdomains get
// MITMed, the forged cert is rejected by CT/pinned hosts, and the site breaks.
// The "."+h guard keeps the match on a label boundary so notyoutube.com does
// not match youtube.com.
func (p *Proxy) isTLSException(hostname string) bool {
	p.invalidTLSHostsMu.RLock()
	defer p.invalidTLSHostsMu.RUnlock()
	if p.invalidTLSHosts[hostname] {
		return true
	}
	for h := range p.invalidTLSHosts {

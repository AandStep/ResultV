...
type Config struct {
	...
	// Dial, when non-nil, replaces the default upstream dialer for BOTH
	// dial paths: the CONNECT/passthrough path AND the internal
	// http.Transport used for MITM'd / plain-HTTP requests. ResultV fork
	// addition — upstream v0.2.1 has no hook for the transport path. See
	// README-FORK.md.

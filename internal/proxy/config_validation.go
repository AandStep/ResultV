...
//
// The engine checks this itself, inside IpcSet (wireguard-go
// device/uapi.go, mergeWithDevice → "headers must not overlap"), and a failure
// there aborts the whole connect with the entire ipcConf as its message. The
// same verdict here names the two headers that collide.
//
...
func validateAWG31Switches(m map[string]interface{}) error {
	for _, name := range []string{awg31RandomTrailersKey, awg31DisableCookiesKey} {

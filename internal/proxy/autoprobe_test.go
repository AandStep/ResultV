...
// The keyed handshake must not run while our own tunnel is using that server.
func TestAutoWireGuardProbeFallsBackWhileTunnelActive(t *testing.T) {
	prevAllowed := autoKeyedWGAllowed.Load()
	t.Cleanup(func() { autoKeyedWGAllowed.Store(prevAllowed) })

	var keylessCalls int
	prevProbe := pingWireGuardProbe
	pingWireGuardProbe = func(ip string, port int) (int64, bool, string) {
		keylessCalls++
		return 42, true, ""
	}
	t.Cleanup(func() { pingWireGuardProbe = prevProbe })

	SetAutoKeyedWGProbe(false)
	rtt, ok, stage, _ := autoWireGuardProbe(config.ProxyEntry{
		IP: "203.0.113.9", Port: 51820, Type: "AMNEZIAWG",
	})
	if keylessCalls != 1 || !ok || rtt != 42 || stage != "udp" {
		t.Fatalf("want one keyless probe reporting udp/42ms, got calls=%d ok=%v rtt=%d stage=%q",

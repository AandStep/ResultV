...
// На мобильной ветке BuildTunnelModeConfig / BuildProxyModeConfig ошибку не
// возвращают — конфиг строится всегда, а негодный узел ловится уже ядром в
// assertCoreAcceptsConfig. Обёртки сохранены, чтобы перенесённые с ПК тесты
// читались там и тут одинаково.
func mustBuildTunnelModeConfig(t *testing.T, cfg EngineConfig) SingBoxConfig {
	t.Helper()
	return BuildTunnelModeConfig(cfg)
}

func mustBuildProxyModeConfig(t *testing.T, cfg EngineConfig) SingBoxConfig {
	t.Helper()
	return BuildProxyModeConfig(cfg)
}

...
//
// Провайдеры раздают авто-секции строками в двух разных формах, и обе
// встречаются в реальных подписках:
//
//  1. Один пул, растянутый на страны: «🇨🇦 impVPN Auto | VLESS»,
//     «🇩🇪 impVPN Auto | HYSTERIA2». Флаг здесь — свойство узла, а не
//     границы группы; общее имя у набора есть («impVPN Auto»).
//  2. Пул на страну: «🇷🇺 RU Auto VLESS», «🇺🇸 US Auto VLESS». Общего имени
//     у набора нет — именно страна и различает секции.
//
// Отсюда две ступени: сначала пробуем общее имя на всём наборе, и только
// когда его нет — раскладываем по ведущему флаг-эмодзи. Разложить по флагам
// сразу значило бы порвать пул формы 1 на одиночек; взять только общее имя —
// схлопнуть секции формы 2 в одну группу и отобрать у пользователя выбор
// страны.
//
// Пул — от двух участников: одно имя ничего не доказывает. Это отличает откат
// от структурной стратегии, где провайдер объявил группу явно и одного члена
// достаточно.
func splitAutoEntriesByName(entries []config.ProxyEntry) ([]AutoGroup, []config.ProxyEntry, bool) {
	...
	// Ступень 1: весь набор под одним именем.
	if name, ok := ExtractAutoGroupName(autoEntries); ok {
		return []AutoGroup{{Name: name, Members: autoEntries}}, individual, true
	}

	// Ступень 2: пул на флаг.
	groups, leftovers := splitAutoEntriesByFlag(autoEntries)
	if len(groups) == 0 {
		return nil, entries, false
	}
	return groups, append(individual, leftovers...), true
}

// splitAutoEntriesByFlag раскладывает авто-записи по ведущему флаг-эмодзи.
// Бакет меньше двух участников или без восстановимого общего имени группой не
// становится и возвращается в leftovers.
func splitAutoEntriesByFlag(autoEntries []config.ProxyEntry) (groups []AutoGroup, leftovers []config.ProxyEntry) {
	const noFlag = "\x00" // сентинел для записей без флага

	var order []string
	buckets := make(map[string][]config.ProxyEntry)
	for _, e := range autoEntries {
		flag, _ := StripLeadingFlagEmoji(e.Name)
		key := flag
		if key == "" {
			key = noFlag
		}

...
type SBDNSServer struct {
	...
	// Predefined засевает статический сервер типа "hosts": имя → готовый
	// список адресов. Нужен пробе пинга: её одноразовый движок работает без
	// платформенного интерфейса libbox, а значит и без резолвера, который тот
	// даёт, — имя узла ему приходится приносить уже разрешённым.

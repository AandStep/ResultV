...
type AppSettings struct {
	...
	// string rather than a pointer precisely because the zero value is already
	// the wanted answer, as with EnableIPv6.
	//
	// "icmp" forces an ICMP echo; "http_get"/"http_head" fetch PingTestURL
	// THROUGH the node itself via a throwaway probe engine. Only the manual
	// ping reads this — the AUTO-group sweep and the health watchdog keep
	// their own tuning, because they run unattended on every connect and a
	// per-node probe engine there would make connecting several times slower.

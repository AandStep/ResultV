...
func TestFetchSubscriptionOverHTTPSSendsHWID(t *testing.T) {
	...
	_ = ts.Client()
	entries, _, _, _, _, _, _, _, err := app.fetchSubscriptionFromURL(ts.URL, false)
	if err != nil {
...
func TestFetchSubscriptionHTTPDefaultRefused(t *testing.T) {
	...
	app := NewApp()
	_, _, _, _, _, _, _, _, err := app.fetchSubscriptionFromURL(ts.URL, false)
	if err == nil {
...
func TestFetchSubscriptionInsecureSuppressesHWID(t *testing.T) {
	...
	app := NewApp()
	entries, _, _, _, _, _, _, _, err := app.fetchSubscriptionFromURL(ts.URL, true)
	if err != nil {
...
func TestFetchSubscriptionImpioJSONFallback(t *testing.T) {
	...
	app := NewApp()
	entries, _, _, _, _, _, _, _, err := app.fetchSubscriptionFromURL(ts.URL+"/api/sub/raw/abc/json", true)
	if err != nil {
...
func TestFetchSubscriptionImpioRawURLNotRewritten(t *testing.T) {
	...
	app := NewApp()
	entries, _, _, _, _, _, _, _, err := app.fetchSubscriptionFromURL(ts.URL+"/api/sub/raw/abc", true)
	if err != nil {
...
func TestFetchSubscriptionSendsConfiguredUserAgentAndDeviceHeaders(t *testing.T) {
	...
	if _, _, _, _, _, _, _, _, err := app.fetchSubscriptionFromURL(ts.URL, true); err != nil {
...
func TestFetchSubscriptionFromURLEmptyBodyReturnsHWIDDiagnostic(t *testing.T) {
	...
	app := NewApp()
	_, _, _, _, _, _, _, _, err := app.fetchSubscriptionFromURL(ts.URL, true)
	if err == nil {
...
func TestFetchSubscriptionFromURLProfileTitleOverridesProvider(t *testing.T) {
	...
	app := NewApp()
	entries, _, _, _, _, _, gotTitle, _, err := app.fetchSubscriptionFromURL(ts.URL, true)
	if err != nil {

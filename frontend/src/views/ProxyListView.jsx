...
function SubscriptionHeaderIcon({ url }) {
  ...
  return (
    <img
      ...
      className="w-10 h-10 rounded-xl object-cover shrink-0 border border-zinc-700/50 bg-zinc-800"
...
export const ProxyListView = () => {
  ...
  const handleDeleteSubscription = async (subMeta) => {
    ...
    const ok = window.confirm(
      t("proxyList.confirmDeleteSubscription", { name: subMeta.name || subMeta.url }),
    );
    if (!ok) return;
  ...
  const handleDeleteManualGroup = async (groupProxies) => {
    ...
    const ok = window.confirm(
      t("proxyList.confirmDeleteMyProxies", { count: groupProxies.length }),
    );
    if (!ok) return;
  ...
  const renderProxyCard = (proxy) => {
    ...
    const vpn = isVpnType(proxy.type);
    const protocolInfo = vpn ? getProtocolLabel(proxy) : proxy.type;

    return (
      <div
        key={proxy.id}
        onClick={() => selectAndConnect(proxy)}
        className={`bg-zinc-900 p-4 rounded-3xl border transition-all flex flex-col cursor-pointer group/card outline-none focus:outline-none focus:ring-0 focus-visible:outline-none ${isActive ? "border-[#00A819] shadow-[0_0_20px_rgba(0,168,25,0.1)]" : "border-zinc-800 hover:border-[#00A819] hover:bg-zinc-800/30"}`}
      >
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded shrink-0 whitespace-nowrap ${vpn ? "bg-[#007E3A]/10 text-[#00A819]" : "bg-zinc-800 text-zinc-300"}`}>
                {protocolInfo}
              </span>
            </div>
        ...
        <div className="flex items-center justify-between mt-auto pt-2 gap-2">
          <div
            className={`text-xs flex items-center shrink-0 ${pings[proxy.id] === "Timeout" || pings[proxy.id] === "Error" ? "text-rose-500" : "text-zinc-500"}`}
          >
                  ...
                  <div className="flex items-stretch gap-3 w-full">
                    ...
                    >
                      ...
                      <div className="flex min-w-0 flex-1 items-center gap-2">
                        <h3
                          className={`text-lg font-bold truncate transition-colors ${isSub ? "text-[#00A819]" : "text-white group-hover/hdr:text-[#00A819]"}`}
                        >
                          {groupName}
                        </h3>
                        ...
                        <ChevronDown
                          className={`ml-auto w-4 h-4 shrink-0 text-zinc-400 transition-transform group-hover/hdr:text-zinc-200 ${isCollapsed ? "-rotate-90" : ""}`}
                          aria-hidden
                        />
                      </div>
                    </button>
                  ...
                  {isSub && (
                    <div className="flex flex-col gap-1.5 pl-0">
                      <div className="flex flex-wrap items-center gap-2 text-xs">
                        <span className="inline-flex items-center rounded-full border border-[#00A819]/35 bg-[#007E3A]/12 px-2.5 py-0.5 font-medium whitespace-nowrap">
                          <span className="text-[#00A819]">
                            {usedTrafficStr}
                          </span>
                        ...
                        {expireLine && (
                          <span className="text-zinc-300">{expireLine}</span>
                        )}
                      </div>

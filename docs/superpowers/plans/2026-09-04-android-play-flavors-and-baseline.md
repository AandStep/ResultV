...
### Task 8: Убрать DNS-adblock из play-сборки

**Решение пересмотрено 2026-09-04.** План предполагал переупаковку с
переформулировкой строк. При исполнении выяснилось, что play-сборка грузит
`adblock_reject.srs` и `geosite-category-ads-all.srs`, а `extra_ads.go` /
`youtube_ads.go` зашивают рекламные домены в бинарь и подключены в `engine.go`
без тега, — то есть текст про «трекеры» разошёлся бы с поведением. Человек
выбрал вырезать DNS-фильтрацию из play целиком (путь AdGuard VPN), поэтому
строки переписывать не понадобилось.

Сделано: тег `no_adblock` (`adblock_rules.go`, `extra_ads.go`, `youtube_ads.go`
уходят из сборки, `adblock_stub.go` встаёт вместо них), константа
`adBlockSupported` гасит три ветки в `engine.go` — пустых списков мало, правило
`reject` без матчеров совпадает со всем трафиком; `validateSRS` вынесен в
`srs_validate.go`, потому что им пользуется Smart-режим; в Kotlin —
...
Блок закрыт 2026-09-06. Сводка и доказательства — в разделе 3.2 спека
`docs/android-pc-sync-and-play-spec.md`, туда же вынесено всё незакрытое.

- [x] Обе Go-конфигурации собираются и проходят тесты
- [x] `internal/filter` отсутствует в зависимостях play-сборки
- [x] Оба флейвора собираются, ставятся и работают (эмулятор Android 16)
- [x] `QUERY_ALL_PACKAGES` убран — состав пикера на реальном телефоне не сверен, см. 4.4 спека
- [x] `targetSdk = 36`, ни один экран не уезжает под системные панели
- [x] `bundlePlayRelease` даёт AAB с выравниванием 16 КБ
- [x] Список ручных заполнений для Play Console составлен — `docs/play-console-submission.md`

Не входило в план, но найдено при приёмке и ждёт решения человека: per-app
блокировка не срабатывает, пока включён браузерный ad-block (раздел 4.1 спека).

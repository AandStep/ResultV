...
- Первое подключение после установки APK регулярно валит `CertSelfTest` по
  5-секундному таймауту (`CA self-test: inconclusive (network error)`): MITM
  сносится, `setHttpProxy` не применяется, а тумблер браузерного ad-block
  остаётся включённым — то есть функция молча выключена на всю сессию. Со
  второго подключения, когда движок фильтра уже в кэше, проходит. Лечится
  либо более щедрым таймаутом, либо повтором самотеста, либо честным
  состоянием тумблера.
...
- В списке приложений per-app правил тап по строке не срабатывает — активен
  только чекбокс.
- MRU набранных доменов (`DomainRulesState.history`, ключ `domainHistory`)
  записывается, но не читается: единственным его потребителем были чипы
  «Недавно использованные», вырезанные в 3.5. Убирается вместе с полем
  модели, ключом JSON и двумя тестами — отложено, чтобы не переписывать
  формат ради этого.
